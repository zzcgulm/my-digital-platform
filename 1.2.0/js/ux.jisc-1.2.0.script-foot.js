 ...
 */
  ...
  $.widget("jisc.collapsible", {
    ...
    options: {
      ...
      expandingClass: 'show_hide--is-expanding',
      collapsingClass: 'show_hide--is-collaping',
    ...
    _create: function() {
      ...
      var $trigger_link = $('<a href="#"></a>');		
      $trigger_link.on('click', function(e) {
        e.preventDefault();
      });
	  
	  //Set up text
	   var $hiddenrow = document.createElement('span');
	   var $hidden_row_text = document.createTextNode("[Open panel below]");
	   $hiddenrow.className = "visuallyhidden";
	   $hiddenrow.appendChild($hidden_row_text);
    ...
    collapse: function(duration) {
	  ...
	  self._collapsible_trigger.children().children().text("[Open panel below]");
      ...
      // Add a class to indicate that the section is in the process of collapsing
      self._container.addClass(self.options.collapsingClass);
      ...
      this._collapsible_target
        .slideUp(duration, function() {
          ...
          self._container
            ...
            .removeClass(self.options.collapsingClass);
        });
    ...
    expand: function(duration) {
      ...
      self._collapsible_trigger.children().children().text("[Close panel below]");
	  ...
	  // Add a class to indicate that the section is in the process of expanding
      self._container.addClass(self.options.expandingClass);
      ...
      this._collapsible_target
        .slideDown(duration, function() {
          ...
          self._container
            ...
            .removeClass(self.options.expandingClass);
        });
...
},{"jquery":"jquery","jquery-ui":3}],6:[function(require,module,exports){
...
$('[data-collapsible]').each(function(index, item) {
  ...
  // from the DOM
  var settings = getDOMConfig($item, options);

  $item.collapsible(settings);
});
},{"../core/js/getDOMConfig":11,"./jquery.jisc.show_hide--menu":5,"jquery":"jquery"}],7:[function(require,module,exports){
(function (global){
/**
 * @fileOverview A jQuery UI Widget to format a list of radio buttons and optional select into a responsive tabbed interface, and carousel on small screens
 * @author Russell Kirkland
 * @name $.ui.cx_contentfilter
 * @requires jQuery, jQuery UI widget factory, PURL, CarouFredSel
 */
...
require('jquery-ui');
require('./vendor/jquery.carouFredSel-6.2.1-packed');
require('jquery-touchswipe');
var _t = require('../core/js/translate');

/**
 * Main content filter plugin
 */

// don't exclude 'a' tags from swipable elements
$.fn.swipe.defaults.excludedElements = ['button', 'input', 'select', 'textarea', '.noSwipe'];

$.widget('ui.cx_contentfilter', {

  options: {
    breakpointOuter: 727,
    initClass: 'initialising',
    bufferWidth: 40,
    nextBtn: {
      label: _t('Next'),
      outerClass: 'next-btn',
      innerClass: 'icon icon-chevron-thin-right'
    },
    prevBtn: {
      label: _t('Prev'),
      outerClass: 'prev-btn',
      innerClass: 'icon icon-chevron-thin-left'
    }
  },
  ...
  _create: function() {
    ...
    // get the vars
    this._container        = $(this.element);
    this._ul          = this._container.find('ul');
    this._li          = this._container.find('li');
    this._more        = null;
    this._moreSubList = null;
    this._selected    = null;
    this._select      = this._container.find('select');
    this._timeoutId   = null;
    this._className   = 'cx__content-filter__';

    this._windowWidth = 0;

    /**
      Throw execptions if minimum structure not found
    */

    if (!this._ul.length) {
      throw 'A <ul> is required for the ContentFilter widget';
    }

    if (!this._li.length) {
      throw 'The <ul> must contain <li> elements for the ContentFilter widget';
    }

    // remove init class
    this._ul.removeClass(this.options.initClass);

    // wrap the ul for carousel padding
    this._ul
      .wrap('<div class="' + this._className + 'outer-wrapper" />')
      .wrap('<div class="' + this._className + 'inner-wrapper" />');




    // whack in some links for easy keyboard focus handling
    this._li.each(function () {
      var $this = $(this);

      $this
        .find('a,>span')
        .addClass(self._className + 'label');

      // Find the closest panel to this link
      var $panel = $this.closest('.ui-tabs-panel');

      if($panel.length > 0) {
        // Append the id of the panel to the link, so the page will scroll to it on reload
        var $link = $this.find('a');
        $link.attr('href', $link.attr('href') + '#' + $panel.attr('id'));
      }

    });

    // attach buttons
    this._container.prepend('<a href="#" id="' + this._className + 'next-btn' + this.uuid + '" class="' + this.options.nextBtn.outerClass + ' ' + this._className + 'scroll-btn"><span class="' + this.options.nextBtn.innerClass + '"><span class="visuallyhidden">' + this.options.nextBtn.label + '</span></span></a>');
    this._container.prepend('<a href="#" id="' + this._className + 'prev-btn' + this.uuid + '" class="' + this.options.prevBtn.outerClass + ' ' + this._className + 'scroll-btn"><span class="' + this.options.prevBtn.innerClass + '"><span class="visuallyhidden">' + this.options.prevBtn.label + '</span></span></a>');

    // click handler for more
    this._container.on('click','.'+ this._className + 'more-group__toggle' , $.proxy(this._moreClick, this));

    // handler for keyboard focus on more link
    this._container.on('focus', '.' + this._className + 'label, select', $.proxy(this._moreKeyboardFocus, this));

    // handler for sort filter changes
    this._select.on('change', $.proxy(this._selectChange, this));

    // Handler for tab changes
    self.element.closest('.ui-tabs').on('cxtabsactivate', function(event, ui) {
      // Reflow the content filter when a tab is activated
      // Make sure that we only reflow the content filter in the currently active tab
    ...
    // call the reflow method
    // @TODO: Why is the reflow method stalled by 10ms? Are we waiting for something else to have happened?
    setTimeout(function() {
      self._reflow();

      self._trigger('afterCreate', {}, {elem:self});
  ...
  _moreClick: function(e) {
    e.preventDefault();
    if($(e.target).parents('.' + this._className + 'more-group').length) {
      this._more.toggleClass('open');
    }
  },

  /**
   * Open or close the more tab
   */
  _moreKeyboardFocus: function(e) {

    if (!this._more) {
      return;
    }
    if ($(e.target).parents('.' + this._className + 'more-group').length) {
      this._more.addClass('has-focus');
    }
    else {
      this._more.removeClass('has-focus');
    }
  ...
  _selectChange: function(e) {
    $('form', this._container).trigger('submit');
  },

  /**
   * Throttle window resize events, call the reflow method
   */
  _resize: function(e) {
    // get the context
    var self = this._self;
    ...
    this._timeoutId = window.setTimeout(function() {
      ...
      self._reflow.call(self);
    }, 250);
  },

  /**
   * Reflow the tabs/carousel on window resize
   * If in tab mode, reinitialise on each call, and reflow based on measurements
   * If in carousel mode, reinitialise the carousel and slide to selected query
   */
  _reflow: function() {
    ...
    // add reflowing class
    this._ul.addClass('reflowing');

    // cleanup previous
    if(this._moreSubList) {
      this._moreSubList.find('li').each(function(){
        $(this).appendTo(self._ul);
      });
    }

    if(this._more) {
      this._more.off('keyup', '.' + this._className + 'label', this._traverseSubMenu);
      this._more.remove();
    }
    ...
    /**
      Begin large screen specific
    */
    if ($(window).width() >= this.options.breakpointOuter)
    {

      // destroy previous carousel
      this._ul.trigger('destroy');

      // reset index, and reattach in right order
      index = this._li.length;
      this._li.detach().appendTo(this._ul).each(function(){
        $(this).data('cx-calculated-width', $(this).width());
      });
      ...
      // check if we're too big, if so, add the more container
      if (width > maxWidth)
      {
        this._more = $('<li class="' + this._className + 'more-group">' +
                  '<a class="' + this._className + 'label ' + this._className + 'more-group__toggle" href="#">' +
                  'More <span class="' + this._className + 'result-count more-total-count"></span> <span class="icon icon-arrow-down"></span><span class="icon icon-chevron-up"></span>' +
                  '</a>' +
                  '</li>');
        this._moreSubList = $('<ul class="' + this._className + 'more-group__sub-list"></ul>');
        this._ul.append(this._more);
        this._more.append(this._moreSubList);
      }

      // we only need to adjust if the li's are wider than our max-width
      if ( maxWidth > 0 ) {
        while (this._getWidth() > maxWidth)
        {
          self._moveIntoSublist(index--);
          if ( index < 0 ) {
            break;
          }
        }
      ...
      // do we have a query, and if so is it in the more block?
      if (this._ul.find('li.' + self._className + 'active-result') && this._more)
      {
        this._more.find('li.' + self._className + 'active-result').each(function() {
          self._selected = $(this).clone();
          self._selected.addClass(self._className + 'selected-group')
            .find('.' + self._className + 'label')
            .append('<a href="' + window.location.pathname + '" class="' + self._className + 'close-selected"><span class="icon icon-close" ></span></a>');
          self._ul.append(self._selected);
          self._more.detach();

          if (self._getWidth() > self._getMaxWidth()) {
            self._moveIntoSublist(index--);
          }
        });
      }

      // handler for traversing the more dropdown
      if ( this._more ) {
        this._more.on('keyup', '.' + this._className + 'label', $.proxy(this._traverseSubMenu, this));
      }
    }
    /**
      End large screen specific
    */

    /**
      Begin small screen specific
    */
    else {
      ...
      // build the carousel for small screens
      this._ul.carouFredSel({
        align: 'left',
        height: 'auto',
        scroll: 2,
        auto: false,
        next: {
            button: '#' + this._className + 'next-btn' + this.uuid
        },
        prev: {
            button: '#' + this._className + 'prev-btn' + this.uuid
        },
        swipe: true
      },
      {
      ...
      // do we have a query?
      if (this._ul.find('li.' + this._className + 'active-result'))
      {
        this._ul.trigger('slideTo', 'li.' + this._className + 'active-result');
      }
    }
    /**
      End small screen specific
    */

    // remove reflowing class
    this._ul.removeClass('reflowing');

  },

  /**
   * Utility function to get outer width of container
   */
  _getWidth: function()
  {
    return this._ul.outerWidth();
  },


  /**
   * Utility function to get maximum allowable width of tabs
   */
  _getMaxWidth: function(index)
  {
    var maxWidth = this._container.outerWidth() - this._select.outerWidth() - this.options.bufferWidth;

    return maxWidth < 0 ? 0 : maxWidth;
  },

  /**
   * Utility function to move li into more sub list
   */
  _moveIntoSublist: function (index)
  {
    var detached = this._li.eq(index - 1);
    this._moreCount += detached.data('result-count');
    this._moreSubList.prepend(detached);
    this._more.find('.more-total-count').text('(' + this._moreCount + ')');
  },

  /**
   * Traverse up and down selected menu
   */
  _traverseSubMenu: function(e) {

    if (e.keyCode !== $.ui.keyCode.UP && e.keyCode !== $.ui.keyCode.DOWN) {
      return;
    }

    var labels = this._more.find('.' + this._className + 'label');
    var focusIndex = labels.index($(e.target));

    if (e.keyCode === $.ui.keyCode.UP && focusIndex > 0) {
      $(labels.get(--focusIndex)).focus();
    } else if (e.keyCode === $.ui.keyCode.DOWN && focusIndex < labels.length) {
      $(labels.get(++focusIndex)).focus();
    }
  },
  ...
  destroy: function() {
    // remove handlers
    this._container.off('focus', '.' + this._className + 'label, select', this._moreKeyboardFocus);
    this._select.off('change', this._selectChange);
...
},{"../core/js/translate":13,"./vendor/jquery.carouFredSel-6.2.1-packed":9,"jquery":"jquery","jquery-touchswipe":2,"jquery-ui":3}],8:[function(require,module,exports){
'use strict';

var $ = require('jquery');
var breakpoints = require('../core/js/breakpoints.js');
require('./jquery.cx.contentfilter');

$('.content-filter__holder').cx_contentfilter({
  breakpointOuter: breakpoints.inner,
  afterCreate: function(event, ui) {

    // If we have a hash in the location
    if(window.location.hash.length > 0) {

      // Find the target of this hash and attempt to find a tab parent
      var $tabs = $(window.location.hash).closest('.ui-tabs');

      // If we've got some tabs
      if($tabs.length > 0) {

        // Wait for the page to have processed the default "jump" to the hash,
        // before then overriding it with a jump to the parent tabs
        setTimeout(function() {
          $.scrollTo($tabs);
        }, 50);
      }
...
},{"../core/js/breakpoints.js":10,"./jquery.cx.contentfilter":7,"jquery":"jquery"}],9:[function(require,module,exports){
/*
...
function getAttributes($element, settings, optionsSpec, path) {
  ...
  $.each(optionsSpec, function(key, value) {
    ...
    if($.isPlainObject(value)) {
      ...
      var data = $element.data(path + key.toLowerCase());
...
/**
 * Simple "dumb" function for providing string translations.
 * Just returns the string right back, this should be replaced with some kind
 * of translation logic
 *
...
},{"./vendor/jquery.uniform.js":17,"jquery":"jquery"}],17:[function(require,module,exports){
/*
...
$.widget('ui.pagenav', {
  ...
  _init: function () {
    ...
    if (nh < wh) {
      ...
      // temp
      var menuItems = $('.filter.is-open .local-nav a'),
      scrollItems = menuItems.map(function(index, value){
        var item = $('#' + value.href.split('#')[1]);
        if (item.length) {
          return item;
        }
      });
      ...
      menuItems.click(function(e) {
        var offsetTop = $('#' + $(this).attr('href').split('#')[1]).offset().top;
  ...
  scrollHandler: function(nav, target, menuItems, scrollItems) {
    var fromTop = this._window.scrollTop();
...
},{"../show-hide/jquery.jisc.show_hide":32}],20:[function(require,module,exports){
'use strict';
var $ = require('jquery');
require('jquery-ui');
...
},{"../core/js/breakpoints":10,"../core/js/getDOMConfig":11,"./jquery.jisc.inpagenav":18,"./jquery.jisc.mobile-inpagenav":19,"conduct.js":1,"jquery":"jquery","jquery-ui":3}],21:[function(require,module,exports){
/**
 * @fileOverview A jQuery UI Widget to add smart search to a search input. Wraps a jQuery UI autocomplete widget, and overrides the _renderMenu method.
 * @author Russell Kirkland
 * @name $.jisc.smartsearch
 * @requires  jQuery, jQuery UI widget factory, jQuery UI Autocomplete, jQuery UI Menu
 *
 * @param object autocompleteOptions    // object containing passthrough options for the jQuery UI autocomplete widget
 * @param string inputSelector          // selector for the search input
 */

// @OPTIMIZE: IE8 needs us to assign this extra dollar sign - why?
var $ = require('jquery');
...
var options = {
  autocompleteOptions: {
    source: '',
    minLength: 0,
    appendTo: '.search-results-dropdown'
  },
...
$.widget('jisc.smartsearch', {
  options: options,

/**
 * The standard jQuery UI _create function
 * This mainly wraps a jQuery UI Autocomplete instance, and customises it's events and _renderItem method
 */
  _create: function() {
    ...
    // get the vars
    this._container   = $(this.element),
    this._form        = this._container.is('form') ? this._container : this._container.find('form'),
    this._input       = this._container.find(this.options.inputSelector),
    this._iconClose   = this._container.find('.close'),
    this._searchTerm;

    // clean list here, as amazingly there's no blur event on the widget
    this._input.on('keydown', $.proxy(this._cleanListItems, this));

    // add click handler to see all results
    this._container.on('click', '.see-all-results', $.proxy(this._triggerForm, this));

    // Add GA tracking events to search result links
    this._on({'click .search-result--item a': this._trackSearchEvent });

    // add the autocomplete widget, with some customisation
    this._input.autocomplete({

      open: function(e, ui){

        self._cleanList().addClass('active');
        // dont's show on no value
        if (!self._input.val()) {
          self._input.autocomplete('close');
        }

        if(!self._input.hasClass('open')) {
          self._input.addClass('open');
        }

        //using the 'open' event to capture the originally typed text
        self._searchTerm = self._input.val();
      },
      close: function(e, ui){

        /*
         * Not sure if bug or feature, but you can't search for the same thing twice in
         * a row with the autocomplete widget.  This was a bug but was fixed.
         */
         ...
         * clean the list
         */
        self._cleanList().removeClass('active');
      },

      focus: function(e, ui) {
        self._cleanList();

        if ( $('#search').find('.search-results-dropdown').position().left < -9000 ) {
          return;
        }

        if (ui.item && ui.item.id) {
          if (ui.item.label) {
            self._input.val(ui.item.label);
          }

          $('.search-results-dropdown li')
            .filter('[data-search-id=' + ui.item.id + ']')
            .addClass('active');
      ...
      response: function(e, ui){
        ...
        /*
         * We shouldn't need this because it is being set
         */
        self._iconClose.show();

        // Do we have results? (1 will just be the no result filler)
        var resultText;

        if(ui.content.length > 1) {
          // take one off the length for all results
          resultText = 'Showing the top ' + (ui.content.length - 1) + ' results for <mark>' + self._input.val() + '</mark>';
        } else {
          // otherwise say no results
          resultText = 'We&rsquo;re sorry, we couldn&rsquo;t find any results for &lsquo;<mark>' + self._input.val() + '</mark>&rsquo;';
        }

        // print out
        $('.search-results__intro').html(resultText);
      ...
      select: function(e, ui){
        ...
        /*
         * This function had an error in it whereby ui.item.id wasn't being set.
         * This has seemed to have been fixed without a fix, so the problem
         * may have been else where.
         */
        if (ui.item && ui.item.id) {
          // get the selected link
          var selectedLink = $('.search-results-dropdown li')
            .filter('[data-search-id=' + ui.item.id + ']')
            .find('a');

          // if it's the all results, trigger the form
          if (selectedLink.hasClass('see-all-results')) {
            self._triggerForm();
          }
          else {
            self._trackSearchEvent(selectedLink.attr('href'));
          }
    ...
    // customise the _renderItem function to give us what we want
    .data('ui-autocomplete')._renderItem = function(ul, item) {

      // create unique counters to separate out results with identical values
      if (this.id) {
        this.id++;
      } else {
        this.id = 1;
      ...
      // regexp the matching search strings
      var result = item.result.replace(new RegExp('(?![^&;]+;)(?!<[^<>]*)(' + $.ui.autocomplete.escapeRegex(this.term) + ')(?![^<>]*>)(?![^&;]+;)', 'gi'), '<mark>$1</mark>');

      // if we have already got one of these, we can just add it to the existing list
      // we need to add a shadow class to the real one though, to help the widget step through correctly with keyboard entry
      var grouping = $('#search').find('.group-' + item.group), visibleClass= '';
      if (grouping.length) {
        grouping
          .find('ul')
          .append('<li data-value="' + item.value + '" data-label="' + item.label + '" data-search-id="' + item.id + '" class="search-result--item ui-menu-item">' + result + '</li>');
        visibleClass = ' search-result--shadow';
      }

      // otherwise we'll create a new one, based on type
      switch (item.resultType) {

        case 'all':

          return $('<article class="duo search-result" />')
            .append('<div class="duo__side"></div>' +
              '<div class="duo__body"><ul><li data-search-id="' + item.id + '" class="search-result--item ui-menu-item">' +
              item.result + '</li></ul></div>')
            .appendTo(ul);
        ...
        default:
          ...
          return $('<article class="duo search-result search-result--has-marker ui-menu-item' + visibleClass + '" />')
            .addClass(item.className)
            .addClass('group-' + item.group)
            .append('<div class="duo__side"><span class="marker">' +
              item.marker + '</span></div><div class="duo__body"><ul><li data-value="' + item.value + '" data-label="' + item.label + '" data-search-id="' + item.id + '" class="search-result--item ui-menu-item">' +
              result + '</li></ul></div>')
            .appendTo(ul);
      }
    ...
    // add the options that were passed into this wrapper
    $.each(this.options.autocompleteOptions, function(key, val){
      self._input.autocomplete('option', key, val);
    });

    // manually trigger the search result value change,
    // as we can't do it from the focus event in the widget
    $('.search-results-dropdown').on('mouseover', $.proxy(this._triggerValueChange, this));

    // close autocomplete on nav flyout or icon close click
    $(window).on('flyoutOpened', $.proxy(this._closeAutocomplete, this));
    this._iconClose.on('click', $.proxy(this._clickCloseAutocomplete, this));

  },

  /**
   * Click handler to track a search event with GA
   */
  _trackSearchEvent: function(e) {
    ...
    // Track the search term with google analytics
    var val = this._input.val();
    if(val.length > 0) {
      _gaq.push(['_trackEvent', 'Search dropdown', 'Keyword', val, 1]);
    }

    // Wait 100ms in the hope that the GA track takes less than this time
    // As recommended on https://support.google.com/analytics/answer/1136920?hl=en-GB
    setTimeout(function() {
      location.href = href;
    }, 100);
  },
  ...
  /**
   * Clear up after jquery ui
   */
  _cleanList: function() {
    this._container.find('ul').removeAttr('style');
    return this._container;
  },

  /**
   * Manually clean list item classes
   */
  _cleanListItems: function() {
    this._container.find('.search-results-dropdown li').removeClass('active');
    return this._container;
  },
   ...
   */
  _triggerForm: function(e) {
    this._input.val(this._searchTerm);
    this._form.trigger('submit');
  },

  /**
   * Manually trigger the search result value change, as we can't do it from the focus event in the widget
   */
  _triggerValueChange: function(e) {
    this._cleanListItems();
    var label = $(e.target).closest('li').data('label');
    if (label) {
      this._input.val(label);
    }
  },

  /**
   * Close autocomplete on nav flyout
   */
  _closeAutocomplete: function(e) {
    this._input.removeClass('open');
    this._input.autocomplete('close');
    this._input.val('');
  },

  _clickCloseAutocomplete: function(e) {
    e.preventDefault();
    this._iconClose.hide();
  ...
  destroy: function() {
    ...
    // remove handlers
    this._input.off('keydown', this._cleanListItems);
    this._container.off('click', this._triggerForm);
    $('.search-results-dropdown').off('mouseover', this._triggerValueChange);
    $(window).off('flyoutOpened', this._closeAutocomplete);
    this._iconClose.off('click', this._clickCloseAutocomplete);

    //destroy it
    this._cleanList().removeClass('active');
...
},{"jquery":"jquery","jquery-ui":3}],22:[function(require,module,exports){
/**
 * @fileOverview Controller for the autosuggest functionality
 */
...
var Conduct = require('conduct.js');
var getDOMConfig = require('../../core/js/getDOMConfig');
var options = require('./jquery.jisc.smartsearch'); // Grab the options spec from the widget

/**
 * Helper function to initialise the smartsearch plugin
 */
function setupSmartSearch() {
  var $search = $('#search');
  var settings = getDOMConfig($search, options);

  $search.smartsearch(settings);
}

/**
 * Helper function to destroy the smartsearch plugin
 */
function destroySmartSearch() {
  if ($('#search').is(':jisc-smartsearch')) {
    $('#search').smartsearch('destroy');
  }
}

/**
 * Initialise / destroy the smartsearch plugin at different breakpoints
 */
var conduct = new Conduct({
  'media_queries': [
    {
      query: 'screen and (min-width:' + breakpoints.inner + 'px)',
      fallback: true,
      match: function() {
        setupSmartSearch();
      },
...
},{"../../core/js/breakpoints":10,"../../core/js/getDOMConfig":11,"./jquery.jisc.smartsearch":21,"conduct.js":1,"jquery":"jquery","jquery-ui":3}],23:[function(require,module,exports){
(function (global){
 ...
 * @author Russell Kirkland
 * @name $.jisc.dropdown
 * @requires jQuery, jQuery UI widget factory, jQuery HoverIntent
 * @event flyoutOpened Triggered on the window to allow other elements to respond
 *
...
require('jquery-ui');
require('../core/vendor/jquery.hoverIntent');

/**
 * Dropdown plugin
 */
$.widget('jisc.dropdown', {
  ...
  _create: function() {
    ...
    // get the vars
    this._container = this.element,
    this._li = this._container.find('[data-dropdown-item]');
    // this._li = this._container.find('li.has-popup');
  ...
  /**
   * Add detachable handlers using the $.proxy method for easy disposal in destroy
   */
  _init: function() {
    // attach handlers
    this
      ...
      .find('a')
      .on('focus', $.proxy(this._tabOn, this))
      .on('blur', $.proxy(this._tabOff, this));

    // listen for flyout events
    // @OPTIMIZE: Rework to use widget factory _on and _off event binders negating the need for $.proxy and simplifying unbinding in the destroy method
    $(window).on('flyoutOpened', $.proxy(this._tabOff, this));

    // Close the menu when we tap outside it
    this._on('#main', { 'click': this._resetAll });
  ...
  /**
   * Get the currently selected element on tab on
   * Reset all li elements
   * Fix the element position if required
   * Add the active class
   * Broadcast global event
   * @param  {Event} e The event which triggered this method call
   */
  _tabOn: function(e) {
    // get the vars
    var link = $(e.target),
  ...
  /**
   * Click handler for the top level items to prevent the first click if the sub nav isnt yet shown.
   * This allows the first "click" on touch devices to show the dropdown, the second click will fire the native click event
   * @param  {Event} e The event which triggered this method call
   */
  _click: function(e) {
  ...
  /**
   * Remove the targets active classes and inline position styles
   * @param  {Event} e The event which triggered this method call
   */
  _tabOff: function(e) {
    this._closestListItem(e.target).removeClass(this.options.hoverClass).find(this.options.menuClass);
  },


  /**
   * Remove all active classes and inline position styles
   * @param  {Event} e The event which triggered this method call
   */
  _resetAll: function(e) {
    this._li.removeClass(this.options.hoverClass).find(this.options.menuClass);
  },

  /**
   * Get the main link item parent from any passed element
   * @param  {Element} el The element from which to begin searching upwards through the DOM
   */
  _closestListItem: function(el) {
    return $(el).is('li') ? $(el) : $(el).closest('.has-popup');
  },
  ...
  destroy: function() {
    ...
    $(window)
      .off('flyoutOpened', this._tabOff)
      .off('resize', this._resetAll);

    //this._off('#main', 'click');
    this._off(this._li, 'click');
...
},{"../core/vendor/jquery.hoverIntent":15,"jquery":"jquery","jquery-ui":3}],24:[function(require,module,exports){
var $ = require('jquery');
require('jquery-ui');
...
},{"jquery":"jquery","jquery-ui":3}],25:[function(require,module,exports){
var $ = require('jquery');
require('jquery-ui');
...
},{"jquery":"jquery","jquery-ui":3}],26:[function(require,module,exports){
/**
...
var $nav = $('[data-dropdown]');
var $navMobile = $('[data-mobilemenu]');
var $navMobileFocussed = $('[data-mobilemenu-focussed]');
...
function destroyDropdowns() {
  if ($nav.is(':jisc-dropdown')) {
    $nav.dropdown('destroy');
  }
}
...
},{"../core/js/breakpoints":10,"../core/js/load-desktop-images":12,"./jquery.jisc.dropdown":23,"./jquery.jisc.mobilemenu":25,"./jquery.jisc.mobilemenu-focussed":24,"conduct.js":1,"jquery":"jquery","jquery-ui":3}],27:[function(require,module,exports){
var $ = require('jquery');
require('jquery-ui');
require('../rollup/jquery.jisc.rollup.js');

$('[data-media-attribution]').rollup({
  breakout: {
    parents: '.region--2-up, .region--3-up',
    match: '.has-media',
    width: '1',
    left: '40',
    right: '0'
...
},{"../rollup/jquery.jisc.rollup.js":30,"jquery":"jquery","jquery-ui":3}],28:[function(require,module,exports){
'use strict';

/**
 * @fileOverview A jQuery UI Widget to provide a dynamically updating pagination control
 * @author Joel Mitchell
 * @name $.jisc.pagination
 ...
 * This is exported with module.exports as well as being passed into the widget
 */
var options = {
  'totalEntries': 100,
  'itemsPerPage': 20,
  ...
  'prevTemplate': ' <span class="ico ico-arrow-left">&lt;</span> <span class="pagination__text">{{label}}</span> ',
  'nextTemplate': ' <span class="pagination__text">{{label}}</span> <span class="ico ico-arrow-right">&gt;</span> ',
  'prevLabel': _t('Previous'),
  'nextLabel': _t('Next'),
  'ellipseText': ' <li class="pagination__item pagination__item--space">...</li> ',
  'prevShowAlways': true,
  'nextShowAlways': true,
  'templates': {
    'pageTemp': ' <li class="pagination__item{{#current}} pagination__item--current{{/current}} {{#current1}} pagination__item--current1{{/current1}} {{#current2}} pagination__item--current2{{/current2}}{{#previous}} pagination__item--previous{{/previous}}{{#next}} pagination__item--next{{/next}}"><a href="#" class="pagination__link" data-id="{{{data}}}">{{{text}}}</a></li> '
  }
};

module.exports = options;

$.widget('jisc.pagination', {

  options: options,

  // Add an underscore to the event prefix to make event names easier to read
  widgetEventPrefix: 'pagination_',
  ...
  _create: function() {
    ...
    self._currentPage = options.currentPage || 0;
    self._totalEntries = options.totalEntries || 1;
    self._itemsPerPage = (!options.itemsPerPage || options.itemsPerPage < 0) ? 1 : options.itemsPerPage;

    // Delegate events to the main element
    // Self executing anonymous function used to generate the necessary eventdata
    // for the Widget factory's _on() method based on passed in options
    self._on((function() {
      var eventdata = {};

      eventdata['click .' + options.linkClass] = self._clickPage;
  ...
  /**
   * Calculate the number of available pages based on the current state
   * @return {int} The number of pages
   */
  _numPages: function() {
    var self = this;

    return Math.ceil(self._totalEntries / self._itemsPerPage);
  },

  /**
   * Calculate start and end point of pagination links depending on
   * self._currentPage and numDisplayEntries.
   * @return {array} Array of start and end values
   */
  _getInterval: function() {
    var self = this,
        options = self.options,
        numEntriesHalf = Math.ceil(options.numDisplayEntries / 2),
        numPages = self._numPages(),
        upperLimit = numPages - options.numDisplayEntries,
        start,
        end;

    if(self._currentPage > numEntriesHalf) {
      start = Math.max(Math.min(self._currentPage - numEntriesHalf, upperLimit), 0);
      end = Math.min(self._currentPage + numEntriesHalf, numPages);
    } else {
      start = 0;
      end = Math.min(options.numDisplayEntries, numPages);
    }
  ...
  /**
   * Main click event handler for pagination items
  ...
  /**
   * Select a page based on it's ID
   * @param  {int} pageId The ID of the page to select
   */
  _selectPage: function(pageId) {
    var self = this;

    self._trigger('render_page_event', null, {
      'current': pageId,
      'totalEntries': self._totalEntries,
      'itemsPerPage': parseInt(self._itemsPerPage, 10)
    });
  ...
  /**
   * Render pagination links in the container element
   */
  _drawLinks: function() {
    var self = this,
      options = self.options,
      ...
      numPages = self._numPages(),
      pagerItems = [],
      edge = options.numEdgeEntries,
      ell = options.ellipseText,
      i;

    // If we don't have enough pages to warrant a pagination element just bail out
    if (numPages < 2) {
      self.element.empty();
      return;
    }
    ...
    if (self._currentPage > 0 || options.prevShowAlways) {
      pagerItems.push(self._renderPagerItem(self._currentPage - 1, {
        text: Mustache.render(options.prevTemplate, {'label': options.prevLabel }),
        data: self._currentPage === 0 ? self._currentPage : (self._currentPage - 1),
        previous: true,
        current: false
      }));
    }
    ...
    // Starting points
    if (interval[0] > 0 && edge > 0) {
      var end = Math.min(edge, interval[0]);

      for (i = 0; i < end; i++) {
        pagerItems.push(self._renderPagerItem(i, {
          text: (i + 1),
          data: i
        }));
      }

      if (edge < interval[0] && ell) {
        pagerItems.push(ell);
      }
    }

    // create interval links
    for (i = interval[0]; i < interval[1]; i++) {
      pagerItems.push(self._renderPagerItem(i, {
        text: (i + 1),
        data: i,
        current1: self._currentPage === (i + 1) || self._currentPage === (i - 1) ? true : false,
        current2: self._currentPage === (i + 2) || self._currentPage === (i - 2) ? true : false
      }));
    }

    // Generate ending points
    if (interval[1] < numPages && edge > 0) {
      if (numPages - edge > interval[1]) {
        pagerItems.push(ell);
      }

      var begin = Math.max(numPages - edge, interval[1]);

      for (i = begin; i < numPages; i++) {
        pagerItems.push(self._renderPagerItem(i, {
          text: (i + 1),
          data: i
        }));
    ...
    // Generate 'Next'-Link
    if (self._currentPage < numPages - 1 || options.nextShowAlways) {
      pagerItems.push(self._renderPagerItem(self._currentPage + 1, {
        text: Mustache.render(options.nextTemplate, {'label': options.nextLabel }),
        data: self._currentPage === (numPages - 1) ? self._currentPage : (self._currentPage + 1),
        next: true,
        current: false
      }));
    }
    ...
    // render it
    self.element.html(pagerItems);
  },

  /**
   * Helper function for rendering a single pager link (or a span tag if it's the current page)
   * @param  {int}    pageId     The ID of the page to generate a pager item for
   * @param  {Object} appendopts Options to pass into the Mustache template
   * @return {String}             HTML representing the pager item
   */
  _renderPagerItem: function(pageId, appendopts) {
    var self = this,
        numPages = self._numPages();

    pageId = pageId < 0 ? 0 : (pageId < numPages ? pageId : numPages - 1); // Normalize page id to sane value
    ...
    if (pageId === self._currentPage) {
      if (appendopts.current !== false) {
        appendopts.current = true;
      }
    }

    return Mustache.render(self.options.templates.pageTemp, appendopts);
  },

  /**
   * Handle updated widget options and re-render the widget
   */
  update: function(value) {
    var self = this;

    self._itemsPerPage = value.itemsPerPage;
    self._totalEntries = value.totalEntries;
    self._currentPage = value.current;
...
},{"../core/js/translate":13,"jquery":"jquery","jquery-ui":3,"mustache":4}],29:[function(require,module,exports){
'use strict';
...
// Grab the options spec from the pagination widget
var options = require('./jquery.jisc.pagination');

$('[data-pagination-ajax]').each(function(index, item) {
  var $item = $(item);
  ...
  $item.pagination(settings);
});

},{"../core/js/getDOMConfig":11,"./jquery.jisc.pagination":28,"jquery":"jquery"}],30:[function(require,module,exports){
/**
 * @fileOverview A jQuery UI Widget to enable a 'rollup' reveal on selected elements
 * @author Russell Kirkland
 * @name $.ui.cx_rollup
 * @requires jQuery, jQuery UI widget factory, jquery.transit
 */
...
/*
 @TODO: If a panel is opened with Enter on .media-open:focus, the ideal outcome would be
 1. Move :focus to close button first (make it the first item inside the panel)
 2. Then tab through other links inside panel
 3. Close the panel when :focus moves out of it
 */
...
$.widget('jisc.rollup', {
  ...
  options: {
    breakout: {
      parents: null,    // parents to look for
      match: null,      // closest to match width of
      width: 1,         // percentage of width to match
      left: 0,          // left margin
      right: 0          // right margin
    },
    title: {
      titleSelector: '[data-media-attribution-title]', // title class to look for
      bottom: 0         // bottom pos of title
    },
    content: {
      contentElem: '[data-media-attribution-info]' // content class to look for (that will be rolled up)
    },
    closeIcon: 'icon-times-circle',
    closeClass: 'media__attribution__close'
  },
  ...
  _create: function() {
    ...
    // get the vars
    this._container = $(this.element);
    this._title = this._container.find(this.options.title.titleSelector);
    this._content = this._container.find(this.options.content.contentElem);
    this._open = this._container.find('[data-media-attribution-open]');
    this._close = null;
    this._width = this._content.outerWidth();
    this._height = null;
    this._delayedCloseId = null;
  },
  ...
  _init: function() {
    ...
    // fix the margin on the bottom element
    this._content.find(':last-child').addClass('last-child');

    this._mediaopen = $('<a />', {
      'href': '#'
    });

    this._mediaopen.on('click', $.proxy(this._openRollup, this));

    //wrap open with a link for tab index
    this._open.wrap(this._mediaopen);

    closeElem = $('<span />')
      .addClass(this.options.closeIcon + ' icon icon--background')
      .on('click', $.proxy(this.closeRollup, this));

    this._content.append($('<div class="' + this.options.closeClass + '"></div>').append(closeElem));
  },


  /**
   * Open rollup on click
   * Scroll to top of rollup if the content goes up beyond the viewport
   * Attach handlers to close on delay or interaction
   */
  _openRollup: function(e) {
    ...
    // get the width
    breakout = this.options.breakout, parent, contentRight = 0, contentBottom;

    if (breakout.parents)
    {
      parent = this._container.parents(breakout.parents);
      if (parent.length)
      {
        this._container.css({
          'left': 'auto',
          'right' : breakout.right
        });
        contentRight = 'auto';
      }
    }
    ...
    // get the height
    this._height = this._content.removeAttr('style').height();

    // adjust the bottom based on the title offset
    contentBottom = parseFloat(this._title.css('bottom')) - parseFloat(this.options.title.bottom);

    // scroll to top of content
    var scrollOffset = this._container.offset().top - this._height - contentBottom;
    if ($(window).scrollTop() > scrollOffset)
    {
      $.scrollTo(scrollOffset, 300);
    ...
    this._content
      ...
      .animate({
        ...
        self._delayedClose(5000);

        // add handlers to mouse leave and enter, for skittish hands
        self._content.on('mouseenter', function(){ $.proxy(self._delayedClose(5000), self); });
        self._content.on('mouseleave', function(){ $.proxy(self._delayedClose(500), self); });
      });
  },

  /**
   * Close the rollup and remove handlers
   */
  closeRollup: function(e) {
    if(typeof(e) !== 'undefined') {
      e.preventDefault();
    }
    ...
    this._content.off('mouseenter');
    this._content.off('mouseleave');
    ...
    this._content
      .animate({
        ...
        self._container.removeAttr('style');
      });
  },

  /**
   Set delayed call to close the rollup
   */
  _delayedClose: function(delay) {
    // get the context
    var self = this._self;
    // set a delayed close
    clearTimeout(this._delayedCloseId);
    this._delayedCloseId = setTimeout(function(){
      self.closeRollup();
    }, delay);
  },
  ...
  destroy: function() {
    ...
    // remove handlers
    this._open.off('click', this._open);
    this._close.off('click', this.closeRollup);
    this._content.off('mouseleave', this.closeRollup);
    this._content.off('mouseenter');
    this._content.off('mouseleave');

    this._content.find(':last-child').removeClass('last-child');
    this._content.find(this.options.closeClass).remove();
...
},{"./vendor/scrollTo.js":31,"jquery":"jquery","jquery-ui":3}],31:[function(require,module,exports){
/**
...
},{"jquery":"jquery","jquery-ui":3}],33:[function(require,module,exports){
// JavaScript Document'use strict';
var $ = require('jquery');
var getDOMConfig = require('../core/js/getDOMConfig');
...
},{"../core/js/getDOMConfig":11,"./jquery.jisc.show_hide":32,"jquery":"jquery"}],34:[function(require,module,exports){
/**
...
$.widget( 'jisc.rwdtable', {
  ...
  _create: function() {
    ...
    var self = this,
      ...
      var outer = self.element.parent('.table-wrap');
      var table = outer.find('table');
      var thead = table.find('thead');
      var tbody = table.find('tbody');
      var tfoot = table.find('tfoot');
      var hdrCols = thead.find('th');
      var bodyRows = tbody.add(tfoot).find('tr');
      var container = o.checkContainer ? $(o.checkContainer) : $('<div class="table-menu"><ul /></div>');
    ...
    if (!o.checkContainer) {
      ...
      menuBtn.click(function(){
        ...
        if ( iconElem.hasClass('icon-arrow-down') ) {
          iconElem.switchClass('icon-arrow-down', 'icon-chevron-up');
        }
        else {
          iconElem.switchClass('icon-chevron-up', 'icon-arrow-down');
        }
      ...
      container.bind('inputChange', function() {
        var hiddenCols = container.find('[type=checkbox]').not(':checked').length,
  ...
  _showHideOptional: function() {
    ...
    var optional = $('.optional', this._table),
...
},{"jquery":"jquery","jquery-ui":3}],35:[function(require,module,exports){
/**
...
$.widget( 'jisc.sortableRwdtable', $.jisc.rwdtable, {
  ...
  _setDefault: function() {
    ...
    var defaultIndex = $('thead th', this.element).index($('th[data-sort-default]'));
      // defaultIndex.find('a').append('<span class="icon ' + iconClass + ' icon--sortable-head"></span>');
    if ( defaultIndex ) {
      this._sortRows(defaultIndex-1);
    }
  ...
  _applyHeadingLinks: function() {
    ...
    $('thead > tr > th', this.element).each(function() {
      ...
      innerLink = $('<a />', {
        href: '#'
      }).on('click', self._sortClick.bind(self));
  ...
  _applyIcon: function(elem) {
   ...
   //  // Add aria-expanded set to true
     // self._collapsible_trigger.children().attr("aria-expanded", "false");
	  ...
	  // Set visually hidden text to open panel
      //self._collapsible_trigger.children().children().text("[Open panel below]");
    $('thead > tr > th:eq("' + (this.currentHeadingIndex+1) + '")', this.element).find('span.icon').remove();
  ...
  _sortRows: function(headingIndex) {
    ...
    var trElems = $('tbody > tr', this.element).detach();

    if ( this.currentHeadingIndex === headingIndex ) {
      this.direction = this.direction * -1;
    }

    this._applyIcon($('thead > tr > th:eq("' + (headingIndex+1) + '")', this.element));
    ...
    trElems.sort(function(a, b) {
      ...
      compareElems = $.map(compareElems, function(elem) {
        ...
        var tdElem = $(elem).find('td:eq("' + headingIndex + '")');
  ...
  _sortClick: function(e) {
    ...
    var index = $('thead > tr > th', this.element).index($(e.currentTarget).parent('th'))-1;
...
},{"jquery":"jquery","jquery-ui":3}],36:[function(require,module,exports){
var $ = require('jquery');
...
},{"./jquery.jisc.rwdtable":34,"./jquery.jisc.sortable-rwdtable":35,"jquery":"jquery"}],37:[function(require,module,exports){
/**
...
$.widget('ui.teaserblocks', {
  ...
  _reset_heights: function () {
    ...
    var $blocks = self.element.find('.block');
...
},{"jquery":"jquery","jquery-ui":3}],38:[function(require,module,exports){
'use strict';
...
},{"../core/js/getDOMConfig":11,"./jquery.teaser.js":37,"jquery":"jquery"}],"jquery":[function(require,module,exports){
/*!
...
},{}]},{},[6,8,14,16,20,22,26,27,29,33,36,38]);
